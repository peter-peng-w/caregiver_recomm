...
class TaskProfile:
    def __init__(self, ctx_size, n_choices):
        # shape: (n_choices, ctx_size+n_choices)
        self.weight = np.concatenate([
            np.random.randn(n_choices, ctx_size),               # standard normal distribution, shape: (n_choices, ctx_size)
            np.random.normal(-4, .5, (n_choices, n_choices))    # normal distribution, shape: (n_choices, n_choices)
            ],
            axis=1
        )

        self.time_gap = lambda: np.random.poisson(10)       # poisson distribution, lambda=10.0
        self.time = datetime.now()                          # current time
        self.time_pass()                                    # next event happen
    ...
    def time_pass(self):
        # compute the time that next event happen
